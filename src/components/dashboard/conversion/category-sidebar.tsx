...
export function CategorySidebar({ active, onSelect }: Props) {
  ...
  return (
    <aside className={cn(
      "h-full border-r border-white/5 bg-[#070b18]/60 backdrop-blur-3xl flex flex-col shrink-0 transition-all duration-500 z-30",
      collapsed ? "w-20" : "w-64"
    )}>
      <div className="p-6 border-b border-white/5 flex items-center justify-between">
        ...
        <Button 
          ...
          className="h-8 w-8 text-muted-foreground/60 hover:text-white hover:bg-white/5 ml-auto rounded-lg"
        >
        ...
        {categories.map((cat) => (
          ...
          >
            ...
            {active === cat.id && collapsed && (
              <div className="absolute right-0 top-3 bottom-3 w-1 bg-white rounded-full shadow-[0_0_10px_white]" />
            )}
          </button>
        ))}
      </div>
        ...
        <div className={cn(
          ...
          {!collapsed && (
            <div className="relative z-10">
              ...
              <p className="text-[8px] font-bold text-muted-foreground/60 uppercase tracking-widest">Latency: 14ms</p>
            </div>
          )}
        </div>
      </div>
